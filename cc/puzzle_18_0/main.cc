...
class CaveMap {
 public:
  ...
  int ShortestKeyPath() {
    return ShortestKeyPathImpl(LocateCell('@').value(), KeySet());
  }
 ...
 private:
  ...
  int ShortestKeyPathImpl(CellCoords start, const KeySet held_keys) {
    if (held_keys.size() == total_keys_) {
      return 0;
    }

    auto subproblem_key = std::make_pair(start, held_keys);
    ...
    std::vector<CellCoords> current_horizon{start};
      ...
      for (const CellCoords cell : current_horizon) {
        ...
        if (std::islower(cell_val) && !held_keys.contains(cell_val)) {
          shortest = std::min(
              shortest,
              current_distance +
                  ShortestKeyPathImpl(cell, held_keys.With(cell_val)));
        } else {
  ...
  int total_keys_ = 0;
  absl::flat_hash_map<std::pair<CellCoords, KeySet>, int> solved_subproblems_;
};
...
int main(int argc, char** argv) {
  ...
  std::cout << cave_map.ShortestKeyPath() << "\n";
  return 0;
}
